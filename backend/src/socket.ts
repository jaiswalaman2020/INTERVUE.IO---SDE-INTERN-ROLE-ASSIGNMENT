...
const connectedStudents: ConnectedStudents = {}; // socket.id => name mapping

function buildResults(poll: IPoll, responses: IResponse[]): IPollResults {
  const result: IPollResults = { answers: {} };
...
async function getPollState(socketId: string | null): Promise<IPollState> {
...
export default function socketHandler(
  ...
  socket.on("register-student", async ({ name }: RegisterStudentPayload) => {
    try {
      if (!name || name.trim() === "") {
      ...
      // Check if name is already taken by another connected student
      for (const [socketId, studentName] of Object.entries(connectedStudents)) {
        if (studentName === name && socketId !== socket.id) {
          ...
          if (existingSocket && existingSocket.connected) {
            ...
            // Socket disconnected but not cleaned up, remove stale entry
  ...
  // Real-time chat
  socket.on("chat:message", async ({ sender, text }: ChatMessagePayload) => {
    try {
      if (sender !== "Teacher") {
  ...
  socket.on(
    "create-poll",
    async ({ text, options, timeLimit }: CreatePollPayload) => {
      try {
        console.log(
          `📝 Create poll event received - Text: ${text}, Options: ${options?.length}, timeLimit: ${timeLimit}`,
        );
        ...
        if (lastPoll) {
          ...
          const remainingSeconds = Math.max(
            0,
            lastPoll.timeLimit - elapsedSeconds,
          );
          ...
          console.log(
            `📊 Poll check - Responses: ${responses.length}, Connected: ${connectedStudentCount}, TimerExpired: ${timerExpired}, CanCreate: ${canCreateNewPoll}`,
          );

          if (!canCreateNewPoll) {
            console.log(
              `❌ Cannot create new poll - students still answering (${connectedStudentCount - responses.length} remaining)`,
            );
  ...
  socket.on(
    "submit-answer",
    async ({ questionId, answer, name }: SubmitAnswerPayload) => {
      try {
        console.log(
          `🗳️ Vote attempt - StudentName: ${name}, SocketId: ${socket.id}, QuestionId: ${questionId}`,
        );
        ...
        console.log(
          `⏱️  Poll timing - Now: ${now}, StartedAt: ${startedAt}, Elapsed: ${elapsedSeconds}s, TimeLimit: ${poll.timeLimit}s, Remaining: ${remainingSeconds}s`,
        );
  ...
  socket.on("request-poll-state", async () => {
    try {
      console.log(
        `📋 Request poll state from: ${socket.data.name || "unknown"} (${socket.id})`,
      );
      const state = await getPollState(socket.id);
      console.log(
        `   Poll state: ${state.poll ? `${state.poll.text}` : "none"}, HasResponded: ${state.hasResponded}, CanAskNew: ${state.canAskNew}, IsActive: ${state.isActive}, RemainingSeconds: ${state.remainingSeconds}`,
      );
  ...
  socket.on("timeout", async ({ questionId }: TimeoutPayload) => {
    try {
      const poll = await Poll.findById(questionId);
      if (!poll) {
        return;
  ...
  // Kick student
  socket.on("kick-student", async ({ name }: KickStudentPayload) => {
    try {
      ...
      const targetSocket = [...io.sockets.sockets.values()].find(
        (s) => (s as CustomSocket).data?.name === name,
      );
